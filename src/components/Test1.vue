<template>
  <div class="table-wrap coltype" style="width: 50%; margin: 0 auto;">
    ...
    <div class="form-group">
      <label>Type</label>
      <input type="text" class="" placeholder="Type" v-model="type">
      <label>Q&A</label>
      <input type="name" class="" placeholder="Question" v-model="qa">
      <button type="submit" class="btn btn-primary" @click="adds()">Add</button>
      <p class="error-smg" v-show="hasError">{{errorsmg}}</p>
    </div>
    <br/>
    <div class="form-group">
    <input type="text" class="" style="width:500px" v-model="search" placeholder="Input your keyword">
    </div>
    <br>
...
export default {
  data () {
    return {
      ...
      type: '',
      qa: '',
      errorsmg: 'Please input type and question!',
      hasError: false
    }
  },
  mounted () {
  ...
  computed: {
    rows () {
      ...
      return this.data.rows.filter(row => {
        let isIncludesOrder = row.order.toString().toLowerCase().includes(this.search.toLowerCase())
        let isIncludesType = row.type.toLowerCase().includes(this.search.toLowerCase())
        let isIncludesQuestion = row.question.toLowerCase().includes(this.search.toLowerCase())
        return isIncludesOrder || isIncludesType || isIncludesQuestion
      }) || []
    }
  },
  methods: {
    adds () {
      if (this.type && this.qa) {
        const question = {
          order: this.data.rows.length + 1,
          type: this.type,
          question: this.qa
        }
        this.data.rows.push(question)
        this.type = ''
        this.qa = ''
        this.hasError = false
      } else {
